...
export class ManageBooksComponent implements OnInit, OnDestroy {
  ...
  displayedColumnsUser: string[] = ['id', 'Cedula', 'Nombre', 'Apellido', 'telefono', 'operation'];
  ...
  deleteConfirm(id: number): void {
    ...
    dialogRef.afterClosed()
      ...
      .subscribe(result => {
        if (result === 1) {
          this.getAllBookData();
          this.snackBarService.showSnackBar('Datos eliminados con éxito');
        } else {
          this.snackBarService.showSnackBar('¡¡Se produjo un error!! Intentar otra vez');
        }
      });
  }
